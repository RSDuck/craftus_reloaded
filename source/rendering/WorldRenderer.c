...
static void renderWorld() {
	...
	DebugUI_Text("T: %u P: %u %d %d %d", world->chunkCache[CHUNKCACHE_SIZE / 2][CHUNKCACHE_SIZE / 2]->tasksRunning,
		     world->chunkCache[CHUNKCACHE_SIZE / 2][CHUNKCACHE_SIZE / 2]->genProgress, workqueue->currentQueue, workqueue->queue[0].length, workqueue->queue[1].length);
}
